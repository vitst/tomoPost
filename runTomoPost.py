...
def main(argv):
    try:
        opts, args = getopt.getopt(argv, "hvld:g:i:c:",
                                   ["help", "version", "dfile=", "list",
                                    "info=",
                                    "check=", "remesh"])
            ...
            try:
                ...
                if 'generator:' in generatorLine:
                    ...
                    try:
                        ...
                        print(
                            'There is no generator %s. Choose one from the list...' % generatorName)
                        sys.exit(2)
                    ...
                    if os.path.isfile(generator.blockMeshFileName):
                        ...
                        while choice != 'y' and choice != 'n':
                            if sys.version_info[0] == 2:
                                choice = raw_input(
                                    'The file `{0:s}` exists already. Do you want to delete the file first? (y/n): '.format(
                                        generator.blockMeshFileName))
                            else:
                                choice = input(
                                    'The file `{0:s}` exists already. Do you want to delete the file first? (y/n): '.format(
                                        generator.blockMeshFileName))
